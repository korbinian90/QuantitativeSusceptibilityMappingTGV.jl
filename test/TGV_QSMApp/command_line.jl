@testitem "TGV_QSM command line tests" begin
    ...
    test_tgv_qsm([phasefile_me_nan, "-t", "[2,4]", "-k", "nomask"])
    ...
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_me, "-o", tmpdir, "-v"])
    
    m = "masking option '0.8' is undefined (Maybe '-k qualitymask 0.8' was meant?)"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_1eco, "-o", tmpdir, "-v", "-k", "0.8"])
    
    m = "masking option 'blub' is undefined"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_1eco, "-o", tmpdir, "-v", "-k", "blub"])
    
    m = "Phase offset determination requires all echo times! (2 given, 3 required)"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_me_5D, "-o", tmpdir, "-v", "-t", "[1,2]", "-e", "[1,2]", "--phase-offset-correction"])
    
    m = "echoes=[1,5]: specified echo out of range! Number of echoes is 3"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_me, "-o", tmpdir, "-v", "-t", "[1,2,3]", "-e", "[1,5]"])
    
    m = "echoes=[1,5} wrongly formatted!"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_me, "-o", tmpdir, "-v", "-t", "[1,2,3]", "-e", "[1,5}"])
    
    m = "Number of chosen echoes is 2 (3 in .nii data), but 5 TEs were specified!"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_me, "-o", tmpdir, "-v", "-t", "[1,2,3,4,5]", "-e", "[1,2]"])
    
    m = "size of magnitude and phase does not match!"
    @test_throws ErrorException(m) TGV_QSM_main(["-p", phasefile_me, "-o", tmpdir, "-v", "-t", "[1,2,3]", "-m", magfile_1eco])
    
    m = "robustmask was chosen but no magnitude is available. No mask is used!"
    @test_logs (:warn, m) match_mode=:any TGV_QSM_main(["-p", phasefile_1eco, "-o", tmpdir])
    ...
    @test_logs (:warn, m) match_mode=:any TGV_QSM_main(["-p", phasefile_me, "-m", magfile_me, "-o", tmpdir, "-t", "[1.1, 1.1, 1.1]", "--phase-offset-correction"])
    
    @test_logs TGV_QSM_main(["-p", phasefile_1eco, "-o", tmpdir, "-m", magfile_1eco]) # test that no warning appears
    
    ## test maskfile
    TGV_QSM_main([phasefile_1eco, "-k", maskfile])
    ...
    TGV_QSM_main([phasefile_me, "-o", phasefile_me_uw, "-t", "[2,4,6]"])
    TGV_QSM_main([phasefile_me_uw, "-o", phasefile_me_uw_wrong, "-t", "[2,4,6]"])
    TGV_QSM_main([phasefile_me_uw, "-o", phasefile_me_uw_again, "-t", "[2,4,6]", "--no-rescale"])
    ...
    TGV_QSM_main([phasefile_1eco, "-o", testpath])
    @test isfile(joinpath(testpath, "qsm.nii"))
    
    testpath = joinpath(tmpdir, "test_name_2")
    fn = joinpath(testpath, "unwrap_name.nii")
    TGV_QSM_main([phasefile_1eco, "-o", fn])
    @test isfile(fn)
    
    testpath = joinpath(tmpdir, "test_name_2")
    gz_fn = joinpath(testpath, "unwrap_name.nii.gz")
    TGV_QSM_main([phasefile_1eco, "-o", gz_fn])
    @test isfile(gz_fn)
    
    ## test .gz input file
    TGV_QSM_main([gz_fn, "-o", joinpath(testpath, "gz_read_test.nii")])
    TGV_QSM_main([gz_fn, "-m", gz_fn, "-o", joinpath(testpath, "gz_read_test.nii")])
    
    
    ## test mcpc3ds output files
    testpath = joinpath(tmpdir, "test5d")
    TGV_QSM_main([phasefile_me_5D, "-o", testpath, "-m", magfile_5D, "-t", "[2,4,6]", "-s"])
    ...
    TGV_QSM_main([phasefile_me, "-o", testpath, "-m", magfile_me, "-t", "[2,4,6]", "--phase-offset-correction", "-s"])
    @test isfile(joinpath(testpath, "corrected_phase.nii"))
    
    ## test B0 output files
    testpath = joinpath(tmpdir, "testB0_1")
    TGV_QSM_main([phasefile_me, "-o", testpath, "-m", magfile_me, "-t", "[2,4,6]", "-B"])
    @test isfile(joinpath(testpath, "B0.nii"))
    
    testpath = joinpath(tmpdir, "testB0_2")
    name = "B0_output"
    TGV_QSM_main([phasefile_me, "-o", testpath, "-m", magfile_me, "-t", "[2,4,6]", "-B", name])
